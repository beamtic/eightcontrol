...
class php_helpers
{
    ...
    private bool $is_called_from_cli;

    public function __construct()
    {
        // Because php_helpers should be loaded early in the composition root (E.g. Index.php)
        // We can use it to establish certain things early, before potential pollution of the environment
        if (
            // Apparently php_sapi_name() is not reliable by itself, so we may need additional checks
            (php_sapi_name() == "cli")
            // Note. http_response_code() only returns false if no code has been set previously;
            // there is a very small chance someone might have called E.g. http_response_code(400) before instantiating php_helpers
            || (false === http_response_code())
            // SERVER_SOFTWARE should only be defined if PHP is responding to a HTTP request
            || (!isset($_SERVER['SERVER_SOFTWARE']))
        ) {
            $this->is_called_from_cli = true;
        } else {
            $this->is_called_from_cli = false;
        }
    ...
    /**
     * Tells whether a script is called from CLI or from a browser
     * @return bool 
     */
    public function is_called_from_cli(): bool
    {
        return $this->is_called_from_cli;
    }
}
